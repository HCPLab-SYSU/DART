...
class DARMT(VisRetNet):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        from dart import DAIR
        from dart.spn import EfficientNetB0Pred
        self.pre_image = DAIR(img_size=448,spn=EfficientNetB0Pred())
    def forward(self, x):
        ret = self.pre_image(x, ret_dict=True)
        x = ret['x']
        # print(x.shape)
        x = super().forward(x)
        return x
